...
google_btn = '''
            <button class="btn-secondary google-btn" type="button" onclick="doGoogleLogin()">
...
css = '''
        .google-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 12px; font-size: 15px; font-weight: 600; padding: 12px; margin-bottom: 25px; border-radius: 10px; border: 1px solid var(--border); background: var(--bg-card); color: var(--text); transition: all 0.2s ease; cursor: pointer; }
        .google-btn:hover { background: rgba(255,255,255,0.05); border-color: #4285F4; }
'''
...
js = '''
    ...
    window.doGoogleLogin = function() {
        ...
        const popup = window.open('', 'Google Sign In', width=,height=,top=,left=);
...
html = html.replace('<div class="divider"><span>or</span></div>', google_btn + '\n            <div class="divider"><span>or</span></div>')
html = html.replace('<div class="divider" id="signup-divider"><span>or</span></div>', google_btn + '\n            <div class="divider" id="signup-divider"><span>or</span></div>')
