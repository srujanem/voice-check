...
class AuthGuardControl(tk.Tk):
    ...
    def start_ngrok(self):
        self._log("\n── Connecting Vercel to your PC (ngrok)... ───────\n")
        self.ngrok_btn.config(state="disabled", text="Connecting...", bg=AMBER)

        def run_ngrok():
            try:
                # Use their static ngrok domain that is hardcoded in Vercel
                tunnel_proc = subprocess.Popen(
                    ["ngrok", "http", "--url=blitz-untimed-yiddish.ngrok-free.dev", "5000"],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    text=True,
                    bufsize=1,
                ...
                self.after(0, self._log, "\n✅ VERCEL IS NOW CONNECTED TO YOUR PC!\n", "#10b981")
                self.after(0, self._log, "You can now use authguard.vercel.app normally.\n", TEXT_SEC)
                self.after(0, lambda: self.ngrok_btn.config(text="🔗 VERCEL LINKED", bg=GREEN))
                    
                tunnel_proc.wait()
                self.after(0, lambda: self.ngrok_btn.config(state="normal", text="🔗 CONNECT VERCEL", bg="#2563eb"))
            except Exception as e:
                self.after(0, lambda: self.ngrok_btn.config(state="normal", text="🔗 CONNECT VERCEL", bg="#2563eb"))
                self.after(0, self._log, f"Error starting ngrok: {e}\n", "#f87171")
